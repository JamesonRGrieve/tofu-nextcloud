...
// over SSH. The provider's resources drive them through this transport.
//
// Like the sibling tofu-wordpress/opnsense/proxmox providers, we invoke the
...
// SSHClient runs remote commands on a Nextcloud host over SSH. Safe for
...
func (e *SSHError) Error() string {
	return fmt.Sprintf("nextcloud ssh %q: exit %d: %s", e.Cmd, e.ExitCode, strings.TrimSpace(e.Stderr))
}
...
func (c *SSHClient) runOnce(remote string, stdin []byte) ([]byte, error) {
	...
	if kerr != nil {
		return nil, fmt.Errorf("nextcloud: materialize ssh identity: %w", kerr)
	}
	defer cleanup()
	if keyPath != "" {
	...
	if err := cmd.Run(); err != nil {
		...
		if ctx.Err() == context.DeadlineExceeded {
			return nil, fmt.Errorf("nextcloud ssh %q: timed out after %s", remote, c.timeout)
		}
...
func (c *SSHClient) identityFile() (path string, cleanup func(), err error) {
	...
	f, err := os.CreateTemp("", "nextcloud-key-*")
	if err != nil {
		return "", noop, err
	}
	name := f.Name()
